...
The scenario: a fictitious database, *Anyterial*, serves the standard OPTIMADE
...
underscores — `_anyt_` is fine, `anyt`, `_Anyt_` and `_anyt` are not.
`_httk_` and `_omdb_` come pre-registered (both under `httk.org`).
...
PREFIX = "_anyt_"
BASE_URL = "https://anyterial.example.org/optimade/defs/properties"
...
def claim_prefix() -> None:
    ...
    for bad in ("anyt", "_Anyt_", "_anyt", "anyt_"):
        try:
...
def build_properties() -> dict[str, PropertyDefinition]:
    ...
    properties = {
        "_anyt_total_energy": PropertyDefinition.from_simple(
            "_anyt_total_energy",
            description="Total energy of the calculation.",
            fulltype="float",
            unit="eV",
        ),
        "_anyt_lattice_vectors": PropertyDefinition.from_simple(
            "_anyt_lattice_vectors",
        ...
        "_anyt_wave_class": PropertyDefinition.from_simple(
            "_anyt_wave_class",
    ...
    served = properties["_anyt_total_energy"].with_implementation(sortable=True, response_default=False)
    print("  overlay on _anyt_total_energy:", served.as_optimade()["x-optimade-implementation"])
    print(
        "  neutral original still has no overlay:",
        "x-optimade-implementation" not in properties["_anyt_total_energy"].as_optimade(),
    )
    print()
    return properties
...
def extend_entry_type(properties: dict[str, PropertyDefinition]) -> EntryTypeDefinition:
    ...
    print("the standard definition is untouched:", "_anyt_total_energy" not in calculations.properties)
...
def emit(extended: EntryTypeDefinition) -> None:
    ...
    lattice_vectors: dict[str, Any] = extended.properties["_anyt_lattice_vectors"].as_optimade()
    print("_anyt_lattice_vectors as an OPTIMADE property definition:")
