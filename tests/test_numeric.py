...
def test_to_numeric_is_plain_float64_ndarray() -> None:
    fv = FracVector.create([["1/3", "2/3"], [1, 2]])
...
def test_to_numeric_accepts_varied_matrix_inputs() -> None:
    ...
    assert to_numeric(FracVector.create([[1, 2], [3, 4]])).tolist() == expected
    assert to_numeric(SurdVector.create([[1, 2], [3, 4]])).tolist() == expected
...
def test_to_numeric_scalar_inputs_return_plain_float() -> None:
    ...
    assert to_numeric(FracScalar.create(F(3, 8))) == 0.375
...
def test_to_numeric_scalar_direct() -> None:
    ...
    assert to_numeric_scalar(FracScalar.create(2)) == 2.0
    assert abs(to_numeric_scalar(SurdVector.sqrt_of(3)) - 3.0**0.5) < 1e-12


def test_to_numeric_scalar_rejects_matrix() -> None:
    with pytest.raises(TypeError):
        to_numeric_scalar(FracVector.create([[1, 2], [3, 4]]))
    with pytest.raises(TypeError):
        to_numeric_scalar(SurdVector.create([[1, 2], [3, 4]]))
