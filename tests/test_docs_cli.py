...
def test_cli_compose_and_filter_and_inventory(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert cli.command(["fetch-inventory", inventory.as_uri(), str(copy), "--expect-project", "core"], context(tmp_path)) == 0
...
def test_cli_compose_release_accepts_tag_and_bare_version(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    for spelling, normalized in (("v1.2.3", "v1.2.3"), ("1.2.4", "v1.2.4")):
        assert cli.command(
            ["compose", "--site", str(site), "--build", str(build), "--release", spelling, "--slug", "core", "--url", "https://x"],
            context(tmp_path),
        ) == 0
...
def test_cli_lock_and_lock_check_use_library(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cli_failure_is_nonzero_and_stderr(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    result = cli.command(["compose", "--dev", "--slug", "core", "--url", "x", "--site", str(tmp_path / "s"), "--build", str(tmp_path / "missing")], context(tmp_path))
    assert result == 1
...
def test_cli_relative_project_dir_resolves_once(
