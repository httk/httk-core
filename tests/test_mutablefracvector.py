...
def test_mutablefracvector_copy_round_trip() -> None:
...
def test_slice_assignment_puts_on_common_denominator() -> None:
    m = MutableFracVector(FracVector([["1/2", "1/2"]]))
    m[0, 1] = F(1, 3)
    assert FracVector(m).simplify() == FracVector([["1/2", "1/3"]])


def test_slice_assignment_fancy_index_rows() -> None:
    m = MutableFracVector(FracVector([[1, 2, 3], [4, 5, 6], [7, 8, 9]]))
...
def test_set_inv_agrees_with_fracvector_inv_various(data: list[list[int]]) -> None:
    ...
    m = MutableFracVector(a)
    m.set_inv()
    assert FracVector(m).simplify() == a.inv().simplify()


def test_set_simplify_keeps_integer_denominator() -> None:
    m = MutableFracVector([[2, 4, 6], [8, 10, 12]], denom=4)
    m.set_simplify()
    ...
    assert FracVector(m) == FracVector([[2, 4, 6], [8, 10, 12]], denom=4).simplify()


def test_set_normalize() -> None:
    m = MutableFracVector([[1, 2], [3, 4]], denom=2)
    m.set_normalize()
...
def test_set_normalize_half() -> None:
    m = MutableFracVector([[1, 3], [5, 7]], denom=4)
