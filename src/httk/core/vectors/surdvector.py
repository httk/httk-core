...
- **Sign is exactly decidable**: refine rational lower/upper bounds on each :math:`\\sqrt r` (via
  :func:`~httk.core.exactmath.integer_sqrt` at increasing precision) until the summed
...
class SurdVector(VectorBackend):
    ...
    def __new__(cls, value: Any = _MISSING_VALUE) -> Self:
        ...
        target = SurdScalar if fv.dim == () else SurdVector
    ...
    def __init__(self, value: Any) -> None:
        ...
        self._set_components({1: fv}, fv.dim)

    def _set_components(self, components: dict[int, FracVector], dim: tuple[int, ...]) -> None:
    ...
    def _from_scalar_grid(grid: Any, dim: tuple[int, ...]) -> "SurdVector":
        ...
        for radicand in radicands:
            ...
            components[radicand] = FracVector(build(grid))
    ...
    def _approx_fracvector(self, prec: fractions.Fraction) -> FracVector:
        ...
        for radicand, comp in self._components.items():
            if radicand == 1:
                ...
                approx = fractions.Fraction(exactmath.integer_sqrt(radicand * m * m), m)
...
class SurdScalar(SurdVector):
        ...
        while True:
            ...
            for radicand, comp in self._components.items():
                ...
                if radicand == 1:
                    ...
                    high += q
                    continue
                root = exactmath.integer_sqrt(radicand * scale_sqr)
    ...
    def cos_degrees(cls, q: Any) -> "SurdScalar | None":
        ...
        :func:`~httk.core.exactmath.any_to_fraction`).
    ...
    def to_decimal(
        ...
        return exactmath._to_decimal(compute, digits, rounding, max_refinements)
