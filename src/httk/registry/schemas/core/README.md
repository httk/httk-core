# Vendored OPTIMADE entry-type definitions
...
These definitions are distributed by the Materials-Consortia under the MIT
...
Run `make optimade-defs` from the repository root to re-fetch these files (and
the `LICENSE`) from the URLs above. This is the only source task that uses the
network; ordinary builds and tests read the committed copies offline. After a
